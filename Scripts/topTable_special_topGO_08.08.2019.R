...
if (params$SILAC == "T"){
  ...
  getTopTable <- function(samples, control, eSet, groupVar, x, ttest, parm) {
    ...
    if (biological_reps[x] > 1){
      ...
      #topTable
      if (parm == "less"){
        topTable_up <- topTable[topTable$topTable >1,]
        topTable_up = topTable_up[order(topTable_up$padj, topTable_up$p,decreasing = TRUE),]
        topTable_down <- topTable[topTable$topTable < -1,]
...
topTables_less <- lapply(1:length(samples), function(x) getTopTable(samples, control, test_eSet, "cell_line", x, "less","less"))
topTables_greater <- lapply(1:length(samples), function(x) getTopTable(samples, control,test_eSet, "cell_line", x, "greater","greater"))
