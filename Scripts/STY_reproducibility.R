...
cc <- lapply (1:length(Conditions), function(x){
  all_quants2 <- all_quants[,grep(Conditions[x],colnames(all_quants))]
  all_quants2$Proteins <- rownames(all_quants)
  all_quants2 <- melt(all_quants2)
  all_quants2$cell_line <- gsub("X\\d_(\\w+)",'\\1',all_quants2$variable)
...
write.csv(cc2, paste0(params$CWD,"/results/",params$cell_line,'_',params$pvalue_cutoff,'P_',params$fdr_cutoff,'FDR/',"CV_",params$cell_line,".csv"))
...
pdf(paste0(params$CWD,"/results/",params$cell_line,'_',params$pvalue_cutoff,'P_',params$fdr_cutoff,'FDR/',"Phos_proteins_CV_violin_plot_",params$cell_line,".pdf"), width = 8)
plot(g)
dev.off()
}