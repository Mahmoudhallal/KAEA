...
## Load the input file
phos_STY_INC1 <- read.delim(params$path_Input)
...
to_keep_STY <- c("Protein", "Amino.acid", "Position", "Phospho..STY..Probabilities", "Reverse", "Potential.contaminant", "Localization.prob",Intensity_columns )
...
for (x in 1:length(params$Conditions_to_sum)){
  ## Loop over every Conditions_to_sum and dont do anything if there is only 1 entry
  if (length(params$Conditions_to_sum[[x]]) > 1){
...
if ("_INC" %in% colnames(all_phos_STY)){
...
## Save the proteins 
# for(x in 1:length(all_batches_STY)) {
#   data <- get(paste0("batch_STY_",x))
#   Exp_short <- data.frame()
#   label <- c('Exp_short')
#   assign(c('Exp_short'), paste0(x,"_",get(paste0("batch_STY_",x))[["cell_line"]]))
#   data[[label]] <- Exp_short 
#   assign(paste0("batch_STY_",x), data)
#   }

## Save the batches in outputs for further graphical representations
batches_all_data_STY <- lapply(1:length(Conditions), function(x){
    replicates <- lapply(1:biological_reps[x], function(y) {
      new <- new_all_batches_STY[[x]][new_all_batches_STY[[x]]$Exp_short == paste0(y,'_',Conditions[x]),]
      new <- new[new$Intensity > 0,]
  })
})
save(batches_all_data_STY, file=paste0(params$CWD,"/results/",params$cell_line,'_',params$pvalue_cutoff,'P_',params$fdr_cutoff,'FDR/',"batches_all_data_",params$cell_line,".Rda"))
...
save(batches_all_phospho_data_STY, file=paste0(params$CWD,"/results/",params$cell_line,'_',params$pvalue_cutoff,'P_',params$fdr_cutoff,'FDR/',"batches_all_phospho_data_",params$cell_line,".Rda"))
...
write.table(eSet_data_STY, paste0(params$CWD,"/results/",params$cell_line,'_',params$pvalue_cutoff,'P_',params$fdr_cutoff,'FDR/',"eSet_data_test1_",params$cell_line,".csv"),sep="\t",row.names = FALSE,quote = FALSE)
